    ...
    try:
        with _connect() as connection:
            cursor = connection.execute(
                ...
                (name.strip(), email.strip().lower(), _hash_password(password, salt), salt.hex(), now),
            )
...
def authenticate_user(email: str, password: str) -> dict[str, Any] | None:
    ...
    return _public_user(row) if hmac.compare_digest(expected, actual) else None
...
def create_token(user_id: int) -> str:
    ...
    with _connect() as connection:
        ...
        connection.execute(
            ...
            (user_id, token_hash, (now + timedelta(days=TOKEN_TTL_DAYS)).isoformat(), now.isoformat()),
        )
    return token
...
def load_sessions(user_id: int) -> list[dict[str, Any]]:
    with _connect() as connection:
        rows = connection.execute(
            "SELECT payload_json FROM test_sessions WHERE user_id = ? ORDER BY id ASC",
            (user_id,),
        ).fetchall()
...
def replace_sessions(user_id: int, sessions: list[dict[str, Any]]) -> None:
    ...
    with _connect() as connection:
        ...
        for session in sessions:
            external_id = str(session.get("id") or secrets.token_hex(8))
            connection.execute(
                "INSERT INTO test_sessions (user_id, external_id, payload_json, updated_at) VALUES (?, ?, ?, ?)",
                (user_id, external_id, json.dumps(session, ensure_ascii=False), now),
            )
